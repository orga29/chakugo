...
def process_workbook(df):
 ...
 for _, row in filtered.iterrows():
     ws.append([row[col_code], row[col_name], row['必要数']])
 ...
 footer_cell.value = f"{datetime.today().strftime('%m/%d')} 着後必要数"
...
uploaded = st.file_uploader("Excelファイルをドラッグ&ドロップまたは選択 (.xlsx)", type="xlsx")
if uploaded:
